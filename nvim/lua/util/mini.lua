...
---@field group? string
---@field [1] string
---@field [2] string|function|wk.Spec
...
function M.ai_whichkey(opts)
  ...
  for name, prefix in pairs(mappings) do
    ...
    ret[#ret + 1] = { prefix, group = name, mode = { "o", "x" } }
    for _, obj in ipairs(objects) do
      local desc = obj.desc
      if prefix:sub(1, 1) == "i" then
        desc = desc:gsub(" with ws", "")
      end
      ret[#ret + 1] = { prefix .. obj[1], desc = obj.desc, mode = { "o", "x" } }
    end
  end
...
function M.pairs(opts)
  -- Create a toggle function for mini pairs
  ...
  vim.keymap.set("n", "<leader>up", toggle_pairs, { desc = "Toggle Mini Pairs" })
