...
license: MIT
metadata:
  source: "https://github.com/DietrichGebert/ponytail/tree/14a0d79548d4de8fc2de95c1b94bb0de63a739d3/skills/ponytail"
...
Use the requested level when the user says `ponytail lite`, `ponytail full`, or
`ponytail ultra`; default to **full**. Stay in that mode for the coding task or
until the user says `stop ponytail` / `normal mode`.
...
Ponytail governs what you build, not how much detail to provide when the user
explicitly asks for analysis. `stop ponytail` / `normal mode`: revert. Level
...
`DietrichGebert/ponytail` → `skills/ponytail/SKILL.md` at commit
`14a0d79548d4de8fc2de95c1b94bb0de63a739d3`. This dotfiles copy is maintained
manually; plugin, command, hook, and multi-harness files were intentionally not
copied. Compare/update with:

```bash
gh api repos/DietrichGebert/ponytail/contents/skills/ponytail/SKILL.md --jq .content | base64 --decode
```
