#!/usr/bin/env python3
"""Archive Chromium bookmark folders into an Obsidian note.

Defaults are tuned for Luan's local setup:
- Chromium user data: ~/Library/Application Support/Chromium
...
def resolve_profile_dir(user_data_dir: Path, profile_name: str | None, profile_dir: str | None) -> str:
    ...
    raise RuntimeError(f"Could not find Chromium profile named '{profile_name}'")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=str(Path.home() / "Library/Application Support/Chromium"),
        help="Chromium user data directory",
    )
    parser.add_argument("--profile-name", help="Chromium profile display name (for example: Work)")
    parser.add_argument("--profile-dir", help="Chromium profile directory basename (for example: Default)")
    ...
    parser.add_argument("--source", default="Browser bookmarks", help="Frontmatter source value")
...
def main() -> int:
    ...
    chromium_user_data = Path(args.chromium_user_data_dir)
    profile_dir = resolve_profile_dir(chromium_user_data, args.profile_name, args.profile_dir)

    bookmarks_path = chromium_user_data / profile_dir / "Bookmarks"
